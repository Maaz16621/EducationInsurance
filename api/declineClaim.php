...
$claimId = $_GET['id'];
$reasonForDecline = $_GET['reason']; // Get the reason for decline from the request parameters

// Update the status and reason for the claim in the database
$sql = "UPDATE claims SET status = 'Declined', reasonForDecline = ? WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("si", $reasonForDecline, $claimId);

if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        // If the claim was successfully declined, return a success message
        echo json_encode(array("message" => "Declined"));
    } else {
    ...
    echo json_encode(array("message" => "Failed to decline claim."));
}
